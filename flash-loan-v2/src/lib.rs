...
pub struct ProviderConfig {
    ...
    /// Alchemy Gas Policy ID for gasless (sponsor-paid) transaction execution
...
impl Default for ProviderConfig {
    fn default() -> Self {
        Self {
            ...
            max_retries: 3,
            rate_limit_rps: 10,
            gas_policy_id: std::env::var("ALCHEMY_GAS_POLICY_ID").ok().filter(|s| !s.is_empty()),
        }
    }
}
...
impl AaveV3Provider {
    /// Build an Alchemy Gas-Policy-sponsored HTTP transport. When
    /// `gas_policy_id` is set the policy header is attached to every RPC
    /// request, instructing Alchemy to sponsor tx gas from the configured
    /// paymaster so the signer wallet never pays gas directly.
    fn build_http(url: &str, gas_policy_id: Option<&str>) -> Result<Http, FlashLoanError> {
        let mut headers = http_header::HeaderMap::new();
        if let Some(pid) = gas_policy_id {
            headers.insert(
                http_header::HeaderName::from_static("x-alchemy-pay-gas-policy-id"),
                http_header::HeaderValue::from_str(pid).map_err(|_| FlashLoanError::Internal("bad gas policy id".into()))?,
            );
        }
        let client = reqwest::Client::builder()
            .default_headers(headers)
            .build()
            .map_err(|e| FlashLoanError::RpcError(format!("reqwest client: {}", e)))?;
        let parsed: Url = url.parse()
            .map_err(|e| FlashLoanError::RpcError(format!("Invalid RPC URL: {}", e)))?;
        Ok(Http::new_with_client(parsed, client))
    }

    pub fn new(config: ProviderConfig) -> Result<Self, FlashLoanError> {
        let http = Self::build_http(&config.rpc_url, config.gas_policy_id.as_deref())?;
    ...
    async fn execute_transaction(
        ...
        // Build transaction
        let calldata = self.pool_contract
            .flash_loan_simple(
            ...
            .calldata()
            .ok_or_else(|| FlashLoanError::ContractError("Empty calldata".into()))?;
        ...
        // Sign and send using SignerMiddleware
        let middleware = ethers::middleware::SignerMiddleware::new(self.provider.clone(), signer.clone());
        let pending = middleware.send_transaction(tx, None).await
        ...
        Ok(FlashLoanResult {
            ...
            fees_paid: params.assets.iter()
                ...
                .collect(),
            gas_used,
            gas_price: gas_price.low_u64() as u128,
...
impl UniswapV3Provider {
    pub fn new(config: ProviderConfig) -> Result<Self, FlashLoanError> {
        let http = AaveV3Provider::build_http(&config.rpc_url, config.gas_policy_id.as_deref())?;
        let provider = Provider::<Http>::new(http);
        let provider = Arc::new(provider);
    ...
    async fn check_pool_liquidity(&self, _amount0: U256, _amount1: U256) -> Result<bool, FlashLoanError> {
        self.rate_limiter.wait_if_needed().await;
...
impl FlashLoanProviderTrait for UniswapV3Provider {
    async fn execute_flash_loan(
        ...
        let (_asset, amount) = params.assets.iter().next()
            .ok_or(FlashLoanError::InvalidParameters("No assets specified".into()))?;
        ...
        let multiplier = (self.config.gas_multiplier * 100.0) as u128;
        let adjusted = gas_price.low_u64() as u128 * multiplier / 100;
        let gas_price = U256::from(adjusted.min(self.config.max_gas_price as u128));
...
impl FlashLoanManager {
    pub fn new(config: ProviderConfig) -> Result<Self, FlashLoanError> {
        ...
        // Aave V3 Provider — mainnet (gas-policy sponsored via Alchemy PayGas)
        let mut aave_config = config.clone();
        aave_config.pool_address = std::env::var("AAVE_V3_POOL_MAINNET")
            .unwrap_or_else(|_| "0x87870Bca3e176D52ac214B0cBe9Ea5EA27F822a9".to_string());
        aave_config.rpc_url = std::env::var("MAINNET_RPC_URL")
        ...
        // Uniswap V3 Provider — mainnet
        let mut uni_config = config.clone();
        uni_config.pool_address = std::env::var("UNISWAP_V3_POOL_MAINNET")
            .unwrap_or_else(|_| "0xE5924299Feb23Ea874242924390450Aa960F8D4F".to_string());
        uni_config.rpc_url = std::env::var("MAINNET_RPC_URL")
            .or_else(|_| std::env::var("ETH_RPC_URL"))
            .unwrap_or_else(|_| "https://eth-mainnet.g.alchemy.com/v2/demo".to_string());
        providers.insert(
    ...
    pub fn get_supported_assets(&self) -> Vec<String> {
        ...
        for _provider in self.providers.values() {
...
/// Default gas limit for flash loan callbacks
pub const DEFAULT_CALLBACK_GAS_LIMIT: u64 = 500_000;
...
/// Flash Loan Router — routes to the best available provider
pub struct FlashLoanRouter {
    providers: Vec<Arc<dyn FlashLoanProviderTrait>>,
}

impl FlashLoanRouter {
    pub fn new(providers: Vec<Arc<dyn FlashLoanProviderTrait>>) -> Self {
        Self { providers }
    }
    ...
    pub async fn execute(
        ...
        for provider in &self.providers {
            if provider.supports_asset(
                params.assets.keys().next().unwrap_or(&String::new())
            ) {
                return provider.execute_flash_loan(params).await;
            }
        }
        Err(FlashLoanError::ProviderUnavailable("No suitable provider".into()))
    }
...
impl MockProvider {
    pub fn new(name: &str, fee: u16, supported_assets: Vec<[u8; 20]>) -> Self {
        Self {
            name: name.to_string(),
            fee,
            supported_assets: supported_assets.into_iter().map(Address::from).collect(),
            gas: 200_000,
...
#[async_trait]
impl FlashLoanProviderTrait for MockProvider {
    async fn execute_flash_loan(
        &self,
    ...
    fn provider_name(&self) -> &'static str {
        // Leak the name string for the 'static lifetime — only for mock/testing
        Box::leak(self.name.clone().into_boxed_str())
    }

    fn supports_asset(&self, asset: &str) -> bool {
        if let Ok(addr) = Address::from_str(asset) {
            self.supported_assets.contains(&addr)
        } else {
            false
        }
    }
    ...
    async fn estimate_gas(&self, _params: &FlashLoanParams) -> Result<u64, FlashLoanError> {
        Ok(self.gas)
    }

    fn validate_params(&self, params: &FlashLoanParams) -> Result<(), FlashLoanError> {
        if params.assets.is_empty() {
            return Err(FlashLoanError::InvalidParameters("No assets".into()));
        }
        Ok(())
    }
}
