...
# Persistent: add to /etc/sysctl.d/99-the-bridge.conf
...
```bash
# Pin NIC queues to dedicated cores (example: eth0 queues 0-7 on cores 0-7)
for i in {0..7}; do
  echo $i > /proc/irq/$(grep "eth0-TxRx-$i" /proc/interrupts | awk '{print $1}' | tr -d ':')/smp_affinity_list
done

# Persistent via irqbalance disable + systemd service
...
### 2.6 Hugepages for DPDK (Optional, for 25Gbps+)
```bash
# Add to kernel cmdline: default_hugepagesz=1G hugepagesz=1G hugepages=16
# Mount: mount -t hugetlbfs nodev /mnt/huge
```

### 2.7 Kernel Parameters (sysctl.d/99-the-bridge.conf)
```bash
...
| `THE_BRIDGE_PEERS` | No | `` | Comma-separated `host:port` for DAG consensus peers (port 4002) |
| `THE_BRIDGE_REPLICAS` | No | `` | Comma-separated `host:port` for WAL sync replicas |
...
| `THE_BRIDGE_DATABASE_URL` | Prod | — | PostgreSQL DSN (`postgres://user:pass@host/db`) |
...
### Generate Secrets
```bash
# Generate 64-byte base64 secrets
...
### 4.1 Systemd Unit (`/etc/systemd/system/the-bridge.service`)
```ini
[Unit]
...
### 4.2 Docker Compose (`docker-compose.prod.yml`)
```yaml
version: '3.8'
...
check "$API/health" "health" || exit 1
check "$API/ready" "ready" || exit 1
check "$API/metrics" "metrics" || exit 1
...
### 5.1 Prometheus Config (`prometheus.yml`)
```yaml
global:
  scrape_interval: 5s
...
### 5.2 Alert Rules (`rules/the-bridge.yml`)
```yaml
groups:
  - name: the-bridge
    interval: 10s
    rules:
      - alert: TheBridgeHighLatency
        ...
        annotations:
          ...
          description: "P99 order latency is {{ $value | humanizeDuration }}"
      ...
      - alert: TheBridgeLowThroughput
        ...
        annotations:
          summary: "TPS < 100K"
          description: "Order throughput is {{ $value | humanize }}/s"
      ...
      - alert: TheBridgeWALLag
        ...
        annotations:
          ...
          description: "Replica lag is {{ $value }} records"
      ...
      - alert: TheBridgeKillSwitchArmed
        ...
        annotations:
          ...
          description: "Threat analyzer has armed the kill switch"
      ...
      - alert: TheBridgeKillSwitchActivated
        ...
        annotations:
          ...
          description: "Engine is in shielded state - no new orders accepted"
...
### 6.1 Sovereign Kill Switch (Immediate)
```bash
...
```
[2024-01-15T10:30:00Z INFO  the_bridge::wal] Recovering WAL from /mnt/nvme/wal
[2024-01-15T10:30:02Z INFO  the_bridge::wal] Recovered 2,847,192 records, last LSN=0x2B8F1A
[2024-01-15T10:30:02Z INFO  the_bridge::consensus] Replaying DAG from height 14,823
[2024-01-15T10:30:05Z INFO  the_bridge::crdt] Merging CRDT state from 3 replicas
```
...
# 4. Reset database (if using PostgreSQL for settlement)
...
| Role | Contact | SLA |
|------|---------|-----|
| **Sovereign Architect** | (redacted) | Immediate |
| **Platform Lead** | (redacted) | 15 min |
| **SRE On-Call** | PagerDuty: the-bridge-prod | 5 min |
| **Regulator Liaison** | (redacted) | 1 hr |
...
|------|---------|--------|---------|
| 2026-07-25 | 1.0 | Architect | Initial production runbook |
