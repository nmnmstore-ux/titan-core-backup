...
use crate::PoolData;
use the_bridge_flash_loan_v2::{
    FlashLoanProvider as FLProvider,
    AaveV3Provider, UniswapV3Provider,
    FlashLoanParams,
    FlashLoanProviderTrait,
    FlashLoanRouter,
};
...
pub struct FlashLoanArbConfig {
    ...
    pub extra_rpc_endpoints: Vec<String>, // additional RPCs for rate limit bypass
...
impl Default for FlashLoanArbConfig {
    fn default() -> Self {
        let is_testnet = std::env::var("NETWORK").unwrap_or_else(|_| "mainnet".into()).to_lowercase() == "testnet";
        let rpc = if is_testnet {
            std::env::var("ETH_RPC_URL").unwrap_or_else(|_| "https://eth-sepolia.g.alchemy.com/v2/demo".into())
        } else {
            std::env::var("ETH_RPC_URL").unwrap_or_else(|_| "https://arb1.arbitrum.io/public".into())
        };
        Self {
            eth_rpc_url: rpc,
            ...
            extra_rpc_endpoints: std::env::var("EXTRA_RPC_ENDPOINTS")
                .ok()
            ...
            max_position_size_eth: 500.0,
            max_concurrent_trades: 50,
            slippage_tolerance_bps: 100,
            max_daily_loss_usd: 10_000.0,
            max_consecutive_failures: 5,
            circuit_breaker_cooldown_secs: 300,
            enabled_chains: vec!["arbitrum".into(), "optimism".into(), "base".into(), "ethereum".into(), "bsc".into()],
            enabled_dexes: vec!["uniswap".into(), "sushiswap".into(), "balancer".into(), "camelot".into(), "zyberswap".into()],
            tracked_tokens: vec![
                "WETH".into(), "USDC".into(), "USDT".into(), "DAI".into(),
                "WBTC".into(), "ARB".into(), "OP".into(), "BAL".into(),
            ],
            notification_webhook: None,
            auto_reinvest: true,
            reinvest_min_balance_eth: 0.01,
            profit_sharing_percent: 10.0,
        }
    }
}
...
pub struct FlashLoanArbExecutor {
    ...
    cached_gas_price_gwei: std::sync::Mutex<f64>,
    cached_gas_price_updated: std::sync::Mutex<std::time::Instant>,
    rpc_client: reqwest::Client,
    rpc_counter: std::sync::atomic::AtomicUsize,
    all_rpc_urls: Vec<String>,
...
impl FlashLoanArbExecutor {
    pub fn new(config: FlashLoanArbConfig, flash_loan_router: Arc<FlashLoanRouter>) -> Self {
        // Build list of all RPC endpoints for rotation
        let mut all_rpc_urls = vec![config.eth_rpc_url.clone()];
        all_rpc_urls.extend(config.extra_rpc_endpoints.iter().cloned());
        if all_rpc_urls.len() == 1 {
            // Add fallback public endpoints if no extras configured
            all_rpc_urls.push("https://eth.llamarpc.com".into());
            all_rpc_urls.push("https://rpc.ankr.com/eth".into());
            all_rpc_urls.push("https://cloudflare-eth.com".into());
            all_rpc_urls.push("https://eth.drpc.org".into());
        }
        ...
        Self {
            ...
            cached_gas_price_gwei: std::sync::Mutex::new(50.0),
            cached_gas_price_updated: std::sync::Mutex::new(std::time::Instant::now()),
            rpc_client: reqwest::Client::builder()
                .pool_max_idle_per_host(20)
                .pool_idle_timeout(std::time::Duration::from_secs(5))
                .tcp_keepalive(std::time::Duration::from_secs(3))
                .tcp_nodelay(true)
                .timeout(std::time::Duration::from_secs(3))
                .build()
                .unwrap_or_default(),
            rpc_counter: std::sync::atomic::AtomicUsize::new(0),
            all_rpc_urls,
    ...
    /// Get next RPC URL using round-robin rotation
    fn next_rpc_url(&self) -> String {
        let idx = self.rpc_counter.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % self.all_rpc_urls.len();
        self.all_rpc_urls[idx].clone()
    }
    ...
    pub async fn execute(&self, opportunity: ArbOpportunity) -> ExecutedTrade {
        ...
        if callback_data.is_empty() {
            warn!("Empty calldata for opportunity {}, skipping", opportunity.id);
            let elapsed = start.elapsed();
            return ExecutedTrade {
                id: trade_id, opportunity_id: opportunity.id,
                chain: opportunity.chain, path: opportunity.path,
                ...
                flash_loan_fee: 0, gross_profit: 0,
                gas_cost: 0, gas_cost_usd: 0.0, net_profit: 0, net_profit_usd: 0.0,
                provider: "InvalidCalldata".into(),
                tx_hash: None, success: false,
                error: Some("Empty calldata — invalid addresses in path".into()),
                executed_at: SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs(),
        ...
        assets.insert(format!("0x{}", hex::encode(token_addr)), opportunity.amount_in);
        let flash_params = FlashLoanParams {
            request_id: Uuid::new_v4(),
            provider: FLProvider::AaveV3,
            assets,
            callback_address: format!("0x{}", hex::encode(token_addr)),
        ...
        let (success, flash_loan_fee, tx_hash_opt, error_msg) = match &flash_result {
            Ok(r) => {
                let total_fees: u128 = r.fees_paid.values().sum();
                (r.success, total_fees, r.tx_hash.clone(), r.error.clone())
            }
            Err(e) => {
        ...
        // Use cached gas price (refreshes every 5s)
        let gas_price_gwei = self.get_cached_gas_price_gwei().await;
        let gas_price_wei = (gas_price_gwei * 1e9) as u128;
        let gas_cost: U256 = flash_result.as_ref()
            .map(|r| r.gas_used as U256 * gas_price_wei)
            .unwrap_or(0);
        let eth_price_usd: f64 = std::env::var("ETH_PRICE_USD")
            .ok().and_then(|v| v.parse().ok()).unwrap_or(2000.0);
        let gas_cost_usd = (gas_cost as f64 / 1e18) * eth_price_usd;
    ...
    /// Encodes as: function executeArbitrage(address[] path, uint256 amountIn, uint256 minAmountOut)
    /// selector = keccak256("executeArbitrage(address[],uint256,uint256)") = 0x8b3b923e
    fn build_arbitrage_calldata(&self, opportunity: &ArbOpportunity) -> Vec<u8> {
        // Security: validate all addresses in path before encoding
        for token in &opportunity.path {
            let addr = self.parse_address_or_zero(token);
            if addr == [0u8; 20] && !token.trim_start_matches("0x").is_empty() {
                warn!("Rejecting opportunity {} — invalid address in path: {}", opportunity.id, token);
                return Vec::new(); // Empty calldata = skip execution
            }
        }
        if opportunity.path.is_empty() {
            warn!("Rejecting opportunity {} — empty path", opportunity.id);
            return Vec::new();
        }

        let mut data = Vec::with_capacity(4 + 128 + 32 + 32);

        // Function selector: bytes4(keccak256("executeArbitrage(address[],uint256,uint256)"))
        data.extend_from_slice(&[0x8b, 0x3b, 0x92, 0x3e]);

        // Offset to path array (always 0x60 = 96 bytes from start of params = 3 words)
        data.extend_from_slice(&[0x00u8; 31]);
        data.push(0x60);

        // amountIn (uint256) — encoded as big-endian 32 bytes
        let amount_in = opportunity.amount_in;
        let mut amount_in_bytes = [0u8; 32];
        amount_in_bytes[16..].copy_from_slice(&amount_in.to_be_bytes());
        data.extend_from_slice(&amount_in_bytes);

        // minAmountOut (uint256) — expected_amount_out * 0.995 (0.5% slippage tolerance)
        // Use integer math to avoid f64 precision loss on large token amounts
        let min_out = opportunity.expected_amount_out.saturating_mul(995) / 1000;
        let mut min_out_bytes = [0u8; 32];
        min_out_bytes[16..].copy_from_slice(&min_out.to_be_bytes());
        data.extend_from_slice(&min_out_bytes);

        // Path array length
        let path_len = opportunity.path.len() as u64;
        let mut path_len_bytes = [0u8; 32];
        path_len_bytes[24..].copy_from_slice(&path_len.to_be_bytes());
        data.extend_from_slice(&path_len_bytes);

        // Path array elements — each is a 20-byte address padded to 32 bytes
        for token in &opportunity.path {
            let addr = self.parse_address_or_zero(token);
            let mut addr_bytes = [0u8; 32];
            addr_bytes[12..].copy_from_slice(&addr);
    ...
    fn parse_address_or_zero(&self, s: &str) -> Address {
        ...
        if clean.is_empty() || clean.len() != 40 {
            if !clean.is_empty() {
                warn!("Invalid address length ({} hex chars): {}", clean.len(), s);
            }
            return [0u8; 20];
        }
        match hex::decode(clean) {
            ...
            Ok(bytes) => {
                warn!("Address decoded to {} bytes (expected 20): {}", bytes.len(), s);
                [0u8; 20]
            }
            Err(_) => {
                warn!("Invalid hex address: {}", s);
                [0u8; 20]
            }
        }
    }

    /// Estimate gas via eth_estimateGas RPC call
    #[allow(dead_code)]
    async fn estimate_gas(&self, to: &str, data: &str, value: u128) -> Option<u64> {
        let payload = serde_json::json!({
            "jsonrpc": "2.0",
        ...
        let gas_hex = data["result"].as_str()?.trim_start_matches("0x");
        u64::from_str_radix(gas_hex, 16).ok()
    }

    /// Get cached gas price — refreshes every 5 seconds only
    async fn get_cached_gas_price_gwei(&self) -> f64 {
        {
            let updated = self.cached_gas_price_updated.lock().unwrap();
            if updated.elapsed() < std::time::Duration::from_secs(5) {
                return *self.cached_gas_price_gwei.lock().unwrap();
            }
        }
        // Fetch fresh gas price via rotating RPC
        let rpc_url = self.next_rpc_url();
        let payload = serde_json::json!({
            "jsonrpc": "2.0", "method": "eth_gasPrice", "params": [], "id": 1,
        });
        let price = if let Ok(resp) = self.rpc_client
            .post(&rpc_url)
            .json(&payload)
            .send()
            .await
        {
            if let Ok(data) = resp.json::<serde_json::Value>().await {
                if let Some(hex_val) = data["result"].as_str() {
                    if let Ok(wei) = u128::from_str_radix(hex_val.trim_start_matches("0x"), 16) {
                        wei as f64 / 1e9
                    } else { 50.0 }
                } else { 50.0 }
            } else { 50.0 }
        ...
        *self.cached_gas_price_gwei.lock().unwrap() = price;
        *self.cached_gas_price_updated.lock().unwrap() = std::time::Instant::now();
        price
    }

    /// Simulate transaction via eth_call before execution
    #[allow(dead_code)]
    async fn simulate_transaction(&self, to: &str, data: &str) -> bool {
        let payload = serde_json::json!({
            "jsonrpc": "2.0",
            "method": "eth_call",
            ...
            .await
        {
            if let Ok(data) = resp.json::<serde_json::Value>().await {
                ...
                return data["result"].as_str().is_some();
            }
        }
        false
    }
...
pub struct FlashLoanArbitrageEngine {
    ...
    pub reinvest_usd: std::sync::Mutex<f64>,
}
...
impl FlashLoanArbitrageEngine {
    pub fn new(config: FlashLoanArbConfig) -> Self {
        ...
        Self {
            ...
            reinvest_usd: std::sync::Mutex::new(0.0),
        }
    }

    fn build_router(config: &FlashLoanArbConfig) -> FlashLoanRouter {
        let mut providers: Vec<Arc<dyn FlashLoanProviderTrait>> = Vec::new();

        // Aave V3 provider — use mainnet RPC from config
        let mut aave_config = the_bridge_flash_loan_v2::get_sepolia_config();
        aave_config.rpc_url = config.eth_rpc_url.clone();
        if let Some(ref pk) = config.private_key {
            aave_config.signer_key = pk.clone();
        }
        match AaveV3Provider::new(aave_config) {
            Ok(provider) => {
                info!("Aave V3 provider initialized (signer: {})",
                    if config.private_key.is_some() { "configured" } else { "none" });
                providers.push(Arc::new(provider));
            }
            Err(e) => {
                warn!("Failed to init Aave V3 provider: {}", e);
            }
        }

        // Uniswap V3 provider
        let mut uni_config = the_bridge_flash_loan_v2::get_sepolia_config();
        uni_config.rpc_url = config.eth_rpc_url.clone();
        if let Some(ref pk) = config.private_key {
            uni_config.signer_key = pk.clone();
        }
        match UniswapV3Provider::new(uni_config) {
            Ok(provider) => {
                info!("Uniswap V3 provider initialized");
                providers.push(Arc::new(provider));
            }
            Err(e) => {
                warn!("Failed to init Uniswap V3 provider: {}", e);
            }
        }

        if providers.is_empty() {
            error!("No flash loan providers available — engine will run in dry-run mode only");
        }
    ...
    pub async fn run(&self) -> Result<(), String> {
        ...
        loop {
            ...
            let all_opportunities = self.detector.scan().await;
            let opp_count = all_opportunities.len();

            // Phase 3: Execute ALL profitable opportunities in PARALLEL
            let mut handles = Vec::new();
            let profitable: Vec<_> = all_opportunities.into_iter()
                .filter(|o| o.is_profitable(self.config.min_profit_usd, self.config.min_profit_bps))
                .take(self.config.max_concurrent_trades as usize)
                .collect();

            for opp in profitable {
                // Apply reinvest bonus
                let mut opp = opp;
                {
                    let mut bonus = self.reinvest_usd.lock().unwrap();
                    if *bonus > 1.0 {
                        let eth_price_usd: f64 = std::env::var("ETH_PRICE_USD")
                            .ok().and_then(|v| v.parse().ok()).unwrap_or(2000.0);
                        let bonus_eth = (*bonus / eth_price_usd).max(0.001);
                        let bonus_wei = (bonus_eth * 1e18) as u128;
                        opp.amount_in = opp.amount_in.saturating_add(bonus_wei);
                        *bonus = 0.0;
                    }
                }

                let executor = self.executor.clone();
                let tracker = self.profit_tracker.clone();
            ...
            for handle in handles {
                if let Ok(trade) = handle.await {
                    if trade.success {
                        info!("✅ PARALLEL PROFIT: ${:.2} | {} -> {} ({}ms)",
                            trade.net_profit_usd, trade.token_in, trade.token_out, trade.duration_ms);
                    } else {
                        warn!("❌ Parallel trade FAILED: {:?}", trade.error);
                    }
                    executed += 1;
                }
            }
            ...
            stats.total_scans += 1;
            stats.total_opportunities_found += opp_count as u64;
            ...
            if self.config.auto_reinvest {
                ...
                if balance >= self.config.reinvest_min_balance_eth {
                    let reinvest_eth = balance * (1.0 - self.config.profit_sharing_percent / 100.0);
                    let eth_price_usd: f64 = std::env::var("ETH_PRICE_USD")
                        .ok().and_then(|v| v.parse().ok()).unwrap_or(2000.0);
                    let reinvest_usd = reinvest_eth * eth_price_usd;
                    {
                        let mut bonus = self.reinvest_usd.lock().unwrap();
                        *bonus += reinvest_usd;
                    }
                    info!("Auto-reinvest: balance={:.4} ETH, adding ${:.2} USD to trade bonus (ETH=${:.0})",
                        balance, reinvest_usd, eth_price_usd);
                }
            }
        }
