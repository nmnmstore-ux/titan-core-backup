...
use crate::smart_router::{SmartOrderRouter, RouteRequest, BrokerRoute};
...
use crate::types::{Order, OrderSide, Track, OrderStatus, OrderType, OrderStyle};
use crate::threshold_crypto::{ThresholdCrypto, DecryptedOrder, EncryptedOrder, ZKProof};
use crate::encrypted_mempool::{EncryptedMempool, EncryptedMempoolOrder, BatchAuctionResult as MempoolBatchAuctionResult};
use crate::ghost_integration::{GhostCloak, BrokerEndpoint, BrokerEvasionStrategy, TimingObfuscation};
...
impl SovereignDarkPool {
    ...
    pub async fn run_auction(&self) -> Result<BatchAuctionResult, String> {
        let orders = self.mempool.get_batch_orders(0).await;
        self.fba_engine.run_auction(orders).await
    }
    ...
    pub async fn submit_order(&self, request: SubmitOrderRequest) -> Result<SubmitOrderResponse, String> {
        let encrypted = self.ghost.encrypt_order(&request.order)?;
        let mempool_order = EncryptedMempoolOrder {
            encrypted_order: encrypted,
            priority: 0,
        };
        self.mempool.add_order(mempool_order).await?;
    ...
    async fn run_mempool_cycle_static(
        ...
        for mempool_order in orders {
            match ghost.create_decryption_share(&mempool_order.encrypted_order) {
                Some(share) => {
                    ...
                    if let Some(decrypted) = decrypted {
                        ...
                        let route_result = router.read().await.route(&route_request, &brokers);
            ...
            let latency = (SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() - start_time) as f64;
...
// Helper types for the API module
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubmitOrderRequest {
    pub order: Order,
    pub track: Track,
...
pub mod api {
    use axum::{Router, Json, routing::post, routing::get};
    use std::sync::Arc;

    use super::{DarkPoolManager, SubmitOrderRequest, SubmitOrderResponse};

    async fn submit_order_handler(
        axum::extract::State(manager): axum::extract::State<std::sync::Arc<super::DarkPoolManager>>,
        axum::Json(payload): axum::Json<SubmitOrderRequest>,
    ) -> axum::response::Json<SubmitOrderResponse> {
        match manager.submit_order(payload.0).await {
            Ok(response) => axum::response::Json(response),
            Err(e) => axum::response::Json(SubmitOrderResponse {
                order_id: "".to_string(),
    ...
    async fn status_handler(
        axum::extract::State(manager): axum::extract::State<std::sync::Arc<super::DarkPoolManager>>,
    ) -> axum::response::Json<super::DarkPoolState> {
        axum::response::Json(manager.get_status().await)
    }

    pub fn create_router(manager: std::sync::Arc<super::DarkPoolManager>) -> axum::Router {
        axum::Router::new()
            .route("/api/v1/darkpool/orders", axum::routing::post(submit_order_handler))
            .route("/api/v1/darkpool/status", axum::routing::get(status_handler))
    }
}
