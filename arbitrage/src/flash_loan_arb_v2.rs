...
impl Default for FlashLoanArbConfig {
    fn default() -> Self {
        Self {
            ...
            enabled_dexes: vec!["uniswap".into(), "sushiswap".into(), "balancer".into(), "curve".into()],
            tracked_tokens: vec![
...
pub struct DexPoolMonitor {
    ...
    rpc_pool: crate::rpc_fallback::RpcPool,
}
...
impl DexPoolMonitor {
    pub fn new(config: FlashLoanArbConfig) -> Self {
        let http_client = reqwest::Client::builder()
            ...
            .build()
            .unwrap_or_default();
        let rpc_pool = crate::rpc_fallback::pool_from_env(&config.eth_rpc_url);
    ...
    pub fn init_default_pools(&self) {
        ...
        let known_pools: Vec<(&str, &str, &str, &str, &str, u32)> = vec![
            ...
            ("ethereum", "sushiswap", "0x1b0a3f34c1e53e7677c83449f2b2e1b8a83e095", "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2", "0xdAC17F958D2ee523a2206206994597C13D831ec7", 30),
            ("ethereum", "balancer", "0x1272Fb911F2A9C1c2B7c4b6D1cF5d1A1d5b9a1a2", "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", 30),
    ...
    pub async fn update_pools(&self) -> Result<u32, String> {
        // Discover new pools on first run
        if *self.update_count.read() == 0 {
            self.discover_pools_into_tracked().await;
        }
        ...
        for pool in &pools {
            ...
            // One healthy endpoint (rotates away from 429/throttled RPCs).
            let rpc = self.rpc_pool.next_healthy();
            if rpc.is_empty() {
                continue;
            }
            ...
            match self.eth_call_slot0(&rpc, &pool.address).await {
                Ok((sqrt_price, tick)) => {
                    let liquidity = match self.eth_call_liquidity(&rpc, &pool.address).await {
                        Ok(liq) => liq,
                        Err(_) => 1_000_000u128,
                    };
                ...
                Err(_) => {
                    // V3 slot0 failed — try V2-style getReserves pool
                    match self.eth_call_getReserves(&rpc, &pool.address, 0).await {
                        Ok((res0, res1, _)) => {
                            let sqrt_price = Self::sqrt_price_from_reserves(res0, res1);
                            let liquidity = Self::liquidity_from_reserves(res0, res1);
                            let pool_data = PoolData::new(
                                pool.address, pool.token_a, pool.token_b,
                                pool.fee, liquidity, sqrt_price, 0,
                            );
                            self.pools.insert(pool_key.clone(), pool_data);
                            self.pool_last_updated.insert(pool_key.clone(), Instant::now());
                            updated += 1;
                            debug!("Fetched pool {} via V2 getReserves", pool_key);
                        }
                        Err(e) => {
                            debug!("Failed to fetch pool {} (V3+V2): {}", pool_key, e);
                        }
                    }
                }
            }
    ...
    /// Call getReserves() on a Uniswap V2-style pool (selector 0x9b6f961e).
    /// Returns (reserve0, reserve1, blockTimestampLast).
    async fn eth_call_getReserves(&self, rpc_url: &str, pool_addr: &Address, block_ts: u32) -> Result<(U256, U256, u32), String> {
        let data = format!("0x9b6f961e{}", hex::encode(block_ts.to_le_bytes()));
        let payload = serde_json::json!({
            "jsonrpc": "2.0",
            "method": "eth_call",
            "params": [{
        ...
        if bytes.len() < 64 {
            return Err("Response too short for getReserves".into());
        }

        let mut r0_bytes = [0u8; 16];
        r0_bytes.copy_from_slice(&bytes[0..16]);
        let res0 = U256::from_be_bytes(r0_bytes);

        let mut r1_bytes = [0u8; 16];
        r1_bytes.copy_from_slice(&bytes[16..32]);
        let res1 = U256::from_be_bytes(r1_bytes);

        let block_ts = u32::from_be_bytes([bytes[32], bytes[33], bytes[34], bytes[35]]);

        Ok((res0, res1, block_ts))
    }

    /// Compute sqrtPriceX96 from pool reserves (used for V2 pools).
    /// sqrtPriceX96 = sqrt(reserve0 * 10^18 / reserve1) * 2^192
    fn sqrt_price_from_reserves(reserve0: U256, reserve1: U256) -> U256 {
        let ratio = reserve0 as f64 / reserve1 as f64;
        let sqrt_ratio = ratio.sqrt();
        let scaled = sqrt_ratio * 1_000_000_000.0_f64 * 2.0_f64.powi(192);
        scaled as U256
    }

    /// Compute V2 pool liquidity from reserves: sqrt(reserve0 * reserve1)
    fn liquidity_from_reserves(reserve0: U256, reserve1: U256) -> U256 {
        (reserve0 as f64 * reserve1 as f64).sqrt() as U256
    }

    /// Discover pools from Uniswap V3 factory for a given token pair
    async fn discover_uniswap_v3_pools(&self, rpc_url: &str, chain: &str) -> Vec<(Address, Address, Address, u32, String)> {
        let uni_factory: Address = [0x1F, 0x98, 0x43, 0x1a, 0xAD, 0xC0, 0x4B, 0x42, 0x13, 0x4d,
                                    0x6b, 0x3a, 0x4c, 0x6F, 0x3B, 0x6F, 0x5a, 0x3d, 0x2c, 0x1d];
        let mut discovered = Vec::new();

        let token_pairs: Vec<(Address, Address, &str, &str)> = vec![
            ([0xA0, 0xb8, 0x69, 0x91, 0xc6, 0x21, 0x8b, 0x36, 0xc1, 0xd1,
             ...
             "USDC", "USDT"),
            ([0xC0, 0x2a, 0xA3, 0x9b, 0x22, 0x3F, 0xE8, 0xD0, 0xA0, 0xe5,
              0xC4, 0xF2, 0x7e, 0xAD, 0x90, 0x83, 0xC7, 0x56, 0xCC, 0x2],
             [0xA0, 0xb8, 0x69, 0x91, 0xc6, 0x21, 0x8b, 0x36, 0xc1, 0xd1,
              0x9D, 0x4a, 0x2e, 0x9E, 0xb0, 0xcE, 0x36, 0x06, 0xeB, 0x48],
             "WETH", "USDC"),
            ([0xC0, 0x2a, 0xA3, 0x9b, 0x22, 0x3F, 0xE8, 0xD0, 0xA0, 0xe5,
        ...
        for fee in [100u32, 500, 3000] {
            for (token_a, token_b, _sa, _sb) in &token_pairs {
                let pool = self.uniswap_v3_get_pool(rpc_url, &uni_factory, token_a, token_b, fee).await;
                if pool != [0u8; 20] {
                    discovered.push((pool, *token_a, *token_b, fee, "uniswap-v3".to_string()));
                }
            }
        }
        discovered
    ...
    /// Call UniswapV3Factory.getPool(address,address,uint24) -> address
    async fn uniswap_v3_get_pool(&self, rpc_url: &str, factory: &Address,
                                   token_a: &Address, token_b: &Address, fee: u32) -> Address {
        let selector = "ad1b7928";
        let ta_padded = format!("{:0>64}", hex::encode(token_a));
        let tb_padded = format!("{:0>64}", hex::encode(token_b));
        let fee_encoded = format!("{:06x}", fee);
        let data = format!("0x{}{}{}{}", selector, ta_padded, tb_padded, fee_encoded);

        let payload = serde_json::json!({
            "jsonrpc": "2.0",
            "method": "eth_call",
            "params": [{
                "to": format!("0x{}", hex::encode(factory)),
                "data": data,
            }, "latest"],
            "id": 1,
        });

        match self.rpc_post_json(rpc_url, &payload).await {
            Ok(result) => {
                let hex_result = result["result"].as_str().unwrap_or("0x");
                let hex_str = hex_result.strip_prefix("0x").unwrap_or(hex_result);
                if hex_str.len() >= 40 {
                    if let Ok(bytes) = hex::decode(&hex_str[24..64]) {
                        if bytes.len() == 20 {
                            let mut addr = [0u8; 20];
                            addr.copy_from_slice(&bytes);
                            return addr;
    ...
    /// Discover pools from Curve registry (known CRV pool addresses)
    async fn discover_curve_pools(&self, chain: &str) -> Vec<(Address, Address, Address, u32, String)> {
        let mut discovered = Vec::new();
        if chain != "ethereum" { return discovered; }

        let usdc = self.parse_eth_address("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48");
        let usdt = self.parse_eth_address("0xdAC17F958D2ee523a2206206994597C13D831ec7");
        let dai = self.parse_eth_address("0x6B175474E89094C44Da98b954EedeAC495271d0F");
        let weth = self.parse_eth_address("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2");

        // 3pool: USDC/USDT/DAI (Curve)
        if let Some(pool_addr) = self.parse_eth_address("0xbEbc5F2C5D4d4D8E2A9cDc7E7F3A28d3e2F9C0dB") {
            if let (Some(ta), Some(tb)) = (usdc, usdt) {
                discovered.push((pool_addr, ta, tb, 100, "curve".to_string()));
            }
        }

        // sUSD/sEUR pool
        if let Some(pool_addr) = self.parse_eth_address("0x06121F7A6F0dD8882Ee5Ee5c5b7B3E89987b5071") {
            if let Some(ta) = usdc {
                if let Some(tb) = dai {
                    discovered.push((pool_addr, ta, tb, 100, "curve".to_string()));
                }
        ...
        // WETH/USDC pool (Curve)
        if let Some(pool_addr) = self.parse_eth_address("0x5F3967d91B82e3576E39d233B407C80bAEd58e5b") {
            if let (Some(ta), Some(tb)) = (weth, usdc) {
                discovered.push((pool_addr, ta, tb, 100, "curve".to_string()));
            }
        }
    ...
    /// Discover pools from Balancer (known balancer pool addresses)
    async fn discover_balancer_pools(&self, chain: &str) -> Vec<(Address, Address, Address, u32, String)> {
        let mut discovered = Vec::new();
        if chain != "ethereum" { return discovered; }
        ...
        // Balancer WETH/USDC/USDT pool
        if let Some(pool_addr) = self.parse_eth_address("0x9d04c6eCcFd1866fAF02DB83a8a78668B529C208") {
            if let (Some(ta), Some(tb)) = (usdt, usdc) {
                discovered.push((pool_addr, ta, tb, 100, "balancer".to_string()));
            }
        }
    ...
    /// Discover additional pools dynamically from factory contracts
    pub async fn discover_pools(&self, chain: &str, rpc_url: &str) -> Vec<(Address, Address, Address, u32, String)> {
        let mut all_discovered = Vec::new();

        if !self.config.enabled_chains.iter().any(|c| c == chain) {
            return all_discovered;
        }

        if chain == "ethereum" && self.config.enabled_dexes.iter().any(|d| d == "uniswap") {
            let uni_pools = self.discover_uniswap_v3_pools(rpc_url, chain).await;
            all_discovered.extend(uni_pools);
        }

        if self.config.enabled_dexes.iter().any(|d| d == "curve") {
            let curve_pools = self.discover_curve_pools(chain).await;
            all_discovered.extend(curve_pools);
        }

        if self.config.enabled_dexes.iter().any(|d| d == "balancer") {
            let bal_pools = self.discover_balancer_pools(chain).await;
            all_discovered.extend(bal_pools);
        }

        let mut seen = std::collections::HashSet::new();
        all_discovered.retain(|(addr, _, _, _, _)| {
            let key = hex::encode(addr);
            if seen.contains(&key) { false } else { seen.insert(key); true }
        });

        if !all_discovered.is_empty() {
            info!("Discovered {} new pools on {}", all_discovered.len(), chain);
        }

        all_discovered
    }

    /// Discover pools and add them to tracked list
    pub async fn discover_pools_into_tracked(&self) -> u32 {
        let rpcs: HashMap<String, String> = [
            ("ethereum".into(), self.config.eth_rpc_url.clone()),
            ("bsc".into(), self.config.bsc_rpc_url.clone()),
            ("polygon".into(), self.config.polygon_rpc_url.clone()),
        ...
        for chain in &self.config.enabled_chains {
            if let Some(rpc_url) = rpcs.get(chain) {
                let discovered = self.discover_pools(chain, rpc_url).await;
                for (addr, ta, tb, fee, dex) in discovered {
                    let pool_key = format!("{}-{}", chain, hex::encode(addr));
                    let already_tracked = self.tracked_pools.read().iter()
                        .any(|p| format!("{}-{}", p.chain, hex::encode(p.address)) == pool_key);
                    if !already_tracked {
                        self.add_pool(chain, &dex, addr, ta, tb, fee, rpc_url);
                        count += 1;
                    }
        ...
        if count > 0 {
            info!("Added {} newly discovered pools", count);
        }
        count
    }
        ...
        for ((token_a, token_b), pools) in &pair_pools {
            ...
            for i in 0..pools.len() {
                for j in i+1..pools.len() {
                    ...
                    let price_i = pool_i.sqrt_price as f64 * pool_i.sqrt_price as f64 / 2.0_f64.powi(192);
                    let price_j = pool_j.sqrt_price as f64 * pool_j.sqrt_price as f64 / 2.0_f64.powi(192);
